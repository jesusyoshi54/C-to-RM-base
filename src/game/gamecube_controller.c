...
s32 osContStartReadDataEx(OSMesgQueue* mq) {
    ...
    if (_osLastSentSiCmd != CONT_CMD_READ_BUTTON) {
        __osPackReadData();
    ...
    _osLastSentSiCmd = CONT_CMD_READ_BUTTON;
    __osSiRelAccess();
...
void osContGetReadDataEx(OSContPadEx* data) {
    ...
    for (i = 0; i < _osContNumControllers; i++, data++) {
...
void __osContGetInitDataEx(u8* pattern, OSContStatus* data) {
    ...
    for (i = 0; i < _osContNumControllers; i++, ptr += sizeof(requestHeader), data++) {
...
//rumble doesn't work on this repo afaik
